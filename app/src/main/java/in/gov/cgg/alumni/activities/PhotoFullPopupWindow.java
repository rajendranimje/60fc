...
import in.gov.cgg.alumni.Constants;
...
public class PhotoFullPopupWindow extends PopupWindow {
    ...
    public PhotoFullPopupWindow(Context ctx, int layout, View v, String imageUrl, Bitmap bitmap) {
        super(((LayoutInflater) ctx.getSystemService(LAYOUT_INFLATER_SERVICE)).inflate(R.layout.pop_up_fill_window, null), ViewGroup.LayoutParams.MATCH_PARENT,
...
//                    .into(photoView);
            if (imageUrl.isEmpty()) {
//                Picasso.with(ctx)
//                        .load(R.drawable.noimage).into(photoView);
                Glide.with(ctx)
...
//                Picasso.with(ctx)
//                        .load(imageUrl)
//                        .error(R.drawable.noimage).into(photoView);
                Glide.with(ctx)
                        .applyDefaultRequestOptions(new RequestOptions()
                                .placeholder(R.drawable.loadingimg_red)
                                .error(R.drawable.user))
